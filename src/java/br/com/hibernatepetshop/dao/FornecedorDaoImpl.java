...
public class FornecedorDaoImpl extends BaseDaoImpl<Fornecedor, Long> implements FornecedorDao {
    ...
    public List<Fornecedor> pesquisarPorNome(String nome, Session sessao) throws HibernateException {
        Query consulta = sessao.createQuery("SELECT DISTINCT(f) FROM Fornecedor f LEFT JOIN FETCH f.enderecos e"
                + " WHERE nome LIKE :nomeHQL");
    ...
    public Fornecedor pesquisarPorNomeProduto(String nomeProduto, Session sessao)throws HibernateException {
        Query consulta = sessao.createQuery("SELECT DISTINCT(f) FROM"
                + " Fornecedor f join fetch f.produtos p "
